...
export async function handleRequest(req: Request, user: { user_id: number; email: string }): Promise<Response> {
  ...
  try {
    ...
    if (path === "/api/tasks" && method === "GET") {
      const { rows } = await query(taskQuery + " WHERE t.user_id = $1 ORDER BY t.created_at DESC", [uid]);
    ...
    if (path === "/api/tasks" && method === "POST") {
      ...
      const { rows } = await query(
        `INSERT INTO tasks (title, description, status, priority_id, category_id, user_id, due_date)
         VALUES ($1,$2,$3,$4,$5,$6,$7) RETURNING id`,
        [body.title, body.description ?? null, body.status ?? "todo",
         body.priority_id ?? null, body.category_id ?? null, uid, body.due_date ?? null],
      );
      ...
      if (method === "PUT") {
        ...
        const { rowCount } = await query(
          `UPDATE tasks SET title=$1, description=$2, status=$3,
           priority_id=$4, category_id=$5, due_date=$6 WHERE id=$7 AND user_id=$8`,
          [body.title, body.description ?? null, body.status,
           body.priority_id ?? null, body.category_id ?? null, body.due_date ?? null, id, uid],
        );
